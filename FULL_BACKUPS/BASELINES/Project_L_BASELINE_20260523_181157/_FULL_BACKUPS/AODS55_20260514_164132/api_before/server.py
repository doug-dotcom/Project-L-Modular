# ============================================================
# SERVER COLLAPSE PHASE ACTIVE
# server.py role:
# - compatibility host
# - route registration
# - runtime bridge
#
# Runtime authority now lives in:
# - orchestration/
# - memory/
# ============================================================
# ============================================================
# SERVER ROLE: compatibility host / runtime glue
# Primary entrypoint: api.main:app
# Routes registered through api.routes.register_routes(app)
# ============================================================
...
from api.routes.direct_agents import (
    ...
    register_direct_agent_handlers,
)
...
register_upload_handler(handle_upload)



register_direct_agent_handlers(
    handle_emily_direct,
    handle_brittany_direct
)

register_memory_handlers(
    handle_memory_audit,
    handle_memory_audit_v2,
    handle_recall_story,
    handle_get_stories
)
...
RUNTIME_STACK = build_legacy_runtime()

RUNTIME_ENGINE = RUNTIME_STACK["runtime_engine"]
TEGAN_RUNTIME = RUNTIME_STACK["tegan_runtime"]
...
# CHAT ROUTE MOVED TO api/routes/chat.py
async def handle_chat(req: ChatRequest):
    ...
    # =====================================================
    # MAJOR TEGAN DYNAMIC DEPLOYMENT
    # =====================================================

    tegan_runtime_result = (
        TEGAN_RUNTIME.process(
            user_msg
        )
    )
...
intent = detect_intent(user_msg)
    ...
    execution_result = try_execution_command(
    ...
    visual_result = try_visual_command(
    ...
    # LEGACY MANUAL ROUTING (BYPASSED)
# LEGACY MANUAL ROUTING (BYPASSED)
# =====================================================
...
# UPLOAD ROUTE MOVED TO api/routes/upload.py
...
# MEMORY ROUTE MOVED TO api/routes/memory.py
async def handle_recall_story(req: ChatRequest):
...
# =========================================================
# DIRECT ROUTE MOVED TO api/routes/direct_agents.py
async def handle_brittany_direct(req: ChatRequest):
...
# =========================================================
# DIRECT ROUTE MOVED TO api/routes/direct_agents.py
async def handle_emily_direct(req: ChatRequest):
...
# GOOGLE ROUTE MOVED TO api/routes/google.py
async def handle_google_status():
