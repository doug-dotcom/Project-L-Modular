...
# ============================================================
# OPENAI FAILURE NORMALIZATION
# ============================================================

def build_openai_failure_response(error):

    try:

        log_runtime_event(
            "openai_failure_normalized",
            {
                "error": str(error)
            }
    ...
    return {
        "reply": (
            "L cognition is temporarily degraded "
            "but the runtime remains operational."
        ),
        "runtime": "tegan_runtime",
        "status": "degraded",
        "cognition": "offline",
...
def validate_response_schema(response):
    ...
    if not isinstance(response, dict):
        ...
        response_payload = {
            "reply": (
                "Runtime returned an invalid "
                "response structure."
            ),
            "runtime": "tegan_runtime",
            "status": "degraded",
            "cognition": "schema_failure"
        }
...
# ============================================================
# RUNTIME EXCEPTION NORMALIZATION
# ============================================================

def normalize_runtime_exception(
    error,
    context="runtime"
):
    ...
    return {
        "reply": (
            "L encountered a runtime exception "
            "but remains operational."
        ),
        "runtime": "tegan_runtime",
        "status": "degraded",
        "cognition": "runtime_exception",
        "context": context,
        "error": str(error)
    }
