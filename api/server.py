...
from core.cognition.durable_task_ledger_audit import load_task_ledger_audit
from core.cognition.recovery_readiness import load_recovery_readiness
...
def health():
    ...
    payload = {
        ...
        "combined_recovery_readiness_ready": True,
...
@app.get("/cognition/recovery-coverage-certification")
def cognition_recovery_coverage_certification(
    page_size: int = 100,
    max_rows: int = 10000,
    x_l_recovery_token: str = Header(default=""),
):
    """Owner-scoped exhaustive durable recovery coverage certification."""
    try:
        return load_recovery_coverage_certification(
            task_store.client,
            x_l_recovery_token,
            page_size=page_size,
            max_rows=max_rows,
        ...
        raise HTTPException(
            503,
            "The durable recovery coverage certification is temporarily unavailable",
        ) from exc


@app.get("/cognition/durable-task-ledger-audit")
def cognition_durable_task_ledger_audit(
    page_size: int = 100,
    max_rows: int = 10000,
    x_l_recovery_token: str = Header(default=""),
):
    """Owner-scoped read-only audit of durable task-journal consistency."""
    try:
        return load_task_ledger_audit(
            task_store.client,
            x_l_recovery_token,
            page_size=page_size,
            max_rows=max_rows,
        ...
        raise HTTPException(
            503,
            "The durable task ledger audit is temporarily unavailable",
        ) from exc


@app.get("/cognition/recovery-readiness")
def cognition_recovery_readiness(
    page_size: int = 100,
    max_rows: int = 10000,
    x_l_recovery_token: str = Header(default=""),
):
    """Read-only ledger and saved-answer checks over one owner-scoped scan."""
    try:
        return load_recovery_readiness(
            task_store.client, x_l_recovery_token, page_size=page_size, max_rows=max_rows,
        )
    except ValueError as exc:
        raise HTTPException(
            400, "A valid recovery token, page size (1–100), and max rows (1–10000) are required",
        ) from exc
    except Exception as exc:
        raise HTTPException(503, "Recovery readiness is temporarily unavailable") from exc
        ...
        try:
            ...
            first_generation_receipt = {
                ...
                "provider_response": dict(
                    (result.get("receipt") or {}).get("provider_response") or {}
                ),
            }
            ...
            if check_evidence:
                ...
                # Layers 67–83 — one bounded repair pass for citation, coverage,
