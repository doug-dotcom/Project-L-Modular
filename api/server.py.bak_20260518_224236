...
# =====================================================
# CANONICAL CATCHALL MEMORY WRITER
# Source-truth raw conversation log
# =====================================================

def write_catchall_memory(role, content, source="chat"):
    ...
    try:
        ...
        catchall_path = ROOT / "memory" / "Catchall.json"

        catchall_path.parent.mkdir(
            parents=True,
            exist_ok=True
        )
        ...
        try:
            log_exception(f"CATCHALL WRITE FAILED: {e}")
        except Exception:
            print(f"CATCHALL WRITE FAILED: {e}")

        return False


# =====================================================
# SUPABASE RAW CATCHALL MEMORY
# Canonical source-truth memory layer
# =====================================================

def write_raw_catchall_supabase(role, content, source="chat"):

    try:

        if not supabase:
            log_error("RAW CATCHALL SKIPPED: Supabase not connected")
            return False
        ...
        print(
            f"RAW CATCHALL WRITE: {role} -> {str(content)[:80]}"
        )
        ...
        try:
            log_exception(f"RAW CATCHALL WRITE FAILED: {e}")
        except Exception:
            print(f"RAW CATCHALL WRITE FAILED: {e}")

        return False


# =====================================================
# RAW CATCHALL MEMORY WRITER
# =====================================================
...
def write_raw_catchall(
    ...
    try:
        ...
        if not supabase:
            ...
            print(
                "RAW CATCHALL: SUPABASE NOT CONNECTED"
            )
