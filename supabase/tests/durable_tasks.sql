...
begin
 ...
 assert not public.l_task_finish_claim_bound(id,gen_random_uuid(),ct,'hash',t.request,'ready','{}');
 assert not public.l_task_progress_claim_bound(id,w,gen_random_uuid(),'hash',t.request,'wrong_token');
 assert not public.l_task_progress_claim_bound(id,w,ct,'changed',t.request,'wrong_hash');
 assert not public.l_task_progress_claim_bound(id,w,ct,'hash','{}'::jsonb,'wrong_request');
 assert public.l_task_progress_claim_bound(id,w,ct,'hash',t.request,'bound_checkpoint');
 assert public.l_task_progress_claim_bound(id,w,ct,'hash',t.request,'test_checkpoint');
 ...
 assert not public.l_task_record_action_claim_bound(id,gen_random_uuid(),ct,'hash',t.request,receipt);
 assert not public.l_task_record_action_claim_bound(id,w,ct,'changed',t.request,receipt);
 assert not public.l_task_record_action_claim_bound(id,w,ct,'hash','{}'::jsonb,receipt);
 assert public.l_task_record_action_claim_bound(id,w,ct,'hash',t.request,receipt);
 assert (select action_receipt from public.l_chat_tasks where request_id=id)=receipt;
 assert public.l_task_record_action_claim_bound(id,w,ct,'hash',t.request,receipt);
 assert not public.l_task_record_action_claim_bound(id,w,ct,'hash',t.request,other_receipt);

 assert not public.l_task_finish_claim_bound(id,w,ct,'hash',t.request,'ready','{"reply":"missing journal"}');
 assert not public.l_task_finish_claim_bound(
   id,w,ct,'hash',t.request,'ready',
   jsonb_build_object('reply','wrong journal','route',jsonb_build_object('action_receipt',other_receipt))
 );
 assert public.l_task_finish_claim_bound(
   id,w,ct,'hash',t.request,'ready',
 ...
 assert not public.l_task_finish_claim_bound(id,w,ct,'hash',t.request,'failed','{}');
 ...
 assert not public.l_task_progress_claim_bound(id,w,ct,'hash',t.request,null);
 ...
 assert not has_function_privilege('service_role','public.l_task_record_action_bound(uuid,uuid,text,jsonb,jsonb)','execute');
 assert not has_function_privilege('service_role','public.l_task_confirm_action_bound(uuid,uuid,text,jsonb,jsonb)','execute');
 assert not has_function_privilege('service_role','public.l_task_progress_bound(uuid,uuid,text,jsonb,text)','execute');
 assert not has_function_privilege('service_role','public.l_task_finish_bound(uuid,uuid,text,jsonb,text,jsonb)','execute');
 assert not has_function_privilege('service_role','public.l_task_confirm_finish_bound(uuid,uuid,text,jsonb,text,jsonb)','execute');
 assert not has_function_privilege('service_role','public.l_task_reject_bound(uuid,uuid,text,jsonb,jsonb)','execute');
 assert not has_function_privilege('anon','public.l_task_reject_bound(uuid,uuid,text,jsonb,jsonb)','execute');

 assert has_function_privilege('service_role','public.l_task_progress_claim_bound(uuid,uuid,uuid,text,jsonb,text)','execute');
 assert has_function_privilege('service_role','public.l_task_record_action_claim_bound(uuid,uuid,uuid,text,jsonb,jsonb)','execute');
 assert has_function_privilege('service_role','public.l_task_confirm_action_claim_bound(uuid,uuid,uuid,text,jsonb,jsonb)','execute');
 assert has_function_privilege('service_role','public.l_task_finish_claim_bound(uuid,uuid,uuid,text,jsonb,text,jsonb)','execute');
 assert has_function_privilege('service_role','public.l_task_confirm_finish_claim_bound(uuid,uuid,uuid,text,jsonb,text,jsonb)','execute');
 assert has_function_privilege('service_role','public.l_task_reject_claim_bound(uuid,uuid,uuid,text,jsonb,jsonb)','execute');
 assert not has_function_privilege('anon','public.l_task_progress_claim_bound(uuid,uuid,uuid,text,jsonb,text)','execute');
 assert not has_function_privilege('authenticated','public.l_task_finish_claim_bound(uuid,uuid,uuid,text,jsonb,text,jsonb)','execute');
