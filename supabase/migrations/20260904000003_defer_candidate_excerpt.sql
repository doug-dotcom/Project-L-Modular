-- Generate legacy transcript excerpts only after the global candidate limit.
...
begin
  ...
  v_sql := format($sql$
    select coalesce(
      jsonb_agg(
        public.project_l_candidate_row(
          candidate_row, table_name, source_role, provenance_evidence, $1
        )
    ...
    from (
      select candidate_row, table_name, source_role, provenance_evidence,
