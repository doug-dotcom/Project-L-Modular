  ...
  loop
    ...
    with candidates as (
      ...
      where r.created_at >= (v_start::timestamp at time zone 'Australia/Brisbane')
        ...
        and not exists (
          select 1
          from public.memory_quarantine q
      ...
      where e.event_date ~ '^\d{4}-\d{2}-\d{2}$'
        and e.event_date >= v_start::text
        and e.event_date < v_end::text
        and coalesce(e.memory_status, '') <> 'QUARANTINED'
        and not exists (
          select 1
          from public.memory_quarantine q
          where q.source_table = 'episodic_memories'
            and q.source_id = e.id::text
            and q.restored_at is null
        )
      ...
      select *
      from candidates
      order by trust desc, rank desc, source
      limit v_limit + 1
    )
    select coalesce(
      jsonb_agg(
    ...
    -- Exact-day recalls should not report a false gap just because generic
    -- semantic terms fail to match. If the bounded semantic pass returns
    -- nothing, retry the same single-day window without FTS filtering.
    -- This remains bounded by v_limit and never broadens beyond the requested day.
    if jsonb_array_length(v_rows) = 0 and p_from = p_through then
      with candidates as (
        select
          'raw_catchall:' || r.id as source,
          r.id as raw_id,
    ...
    v_months := v_months || jsonb_build_array(
      jsonb_build_object(
        ...
        'rows', case when jsonb_array_length(v_rows) > v_limit then v_rows - v_limit else v_rows end,
