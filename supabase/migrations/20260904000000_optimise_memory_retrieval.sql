-- Step 8: keep recall scoring in Rhee, but let Postgres select a bounded
-- candidate set instead of transporting the entire memory corpus.

alter table public.raw_catchall
  add column if not exists retrieval_fts tsvector
  generated always as (
    to_tsvector('simple'::regconfig, coalesce(content, ''))
  ) stored;

create index if not exists raw_catchall_retrieval_fts_idx
  on public.raw_catchall using gin (retrieval_fts);

alter table public.memory_family
  add column if not exists retrieval_fts tsvector
  generated always as (to_tsvector('simple'::regconfig,
    coalesce(content, '') || ' ' || coalesce(primary_subject, '') || ' ' ||
    coalesce(subjects::text, '') || ' ' || coalesce(relationships::text, '') || ' ' ||
    coalesce(values::text, '') || ' ' || coalesce(patterns::text, '') || ' ' ||
    coalesce(preferences::text, '')
  )) stored;
create index if not exists memory_family_retrieval_fts_idx
  on public.memory_family using gin (retrieval_fts);

alter table public.memory_identity
  add column if not exists retrieval_fts tsvector
  generated always as (to_tsvector('simple'::regconfig,
    coalesce(content, '') || ' ' || coalesce(primary_subject, '') || ' ' ||
    coalesce(subjects::text, '') || ' ' || coalesce(relationships::text, '') || ' ' ||
...
create index if not exists memory_identity_retrieval_fts_idx
  on public.memory_identity using gin (retrieval_fts);
...
create index if not exists memory_relationships_retrieval_fts_idx
  on public.memory_relationships using gin (retrieval_fts);

alter table public.memory_recovery
  add column if not exists retrieval_fts tsvector
  generated always as (to_tsvector('simple'::regconfig,
    coalesce(content, '') || ' ' || coalesce(primary_subject, '') || ' ' ||
    coalesce(subjects::text, '') || ' ' || coalesce(relationships::text, '') || ' ' ||
...
create index if not exists memory_recovery_retrieval_fts_idx
  on public.memory_recovery using gin (retrieval_fts);

alter table public.memory_health
  add column if not exists retrieval_fts tsvector
  generated always as (to_tsvector('simple'::regconfig,
    coalesce(content, '') || ' ' || coalesce(primary_subject, '') || ' ' ||
    coalesce(subjects::text, '') || ' ' || coalesce(relationships::text, '') || ' ' ||
...
create index if not exists memory_health_retrieval_fts_idx
  on public.memory_health using gin (retrieval_fts);

alter table public.memory_project_l
  add column if not exists retrieval_fts tsvector
  generated always as (to_tsvector('simple'::regconfig,
    coalesce(content, '') || ' ' || coalesce(primary_subject, '') || ' ' ||
    coalesce(subjects::text, '') || ' ' || coalesce(relationships::text, '') || ' ' ||
...
create index if not exists memory_project_l_retrieval_fts_idx
  on public.memory_project_l using gin (retrieval_fts);

alter table public.memory_general
  add column if not exists retrieval_fts tsvector
  generated always as (to_tsvector('simple'::regconfig,
    coalesce(content, '') || ' ' || coalesce(primary_subject, '') || ' ' ||
    coalesce(subjects::text, '') || ' ' || coalesce(relationships::text, '') || ' ' ||
    coalesce(values::text, '') || ' ' || coalesce(preferences::text, '')
  )) stored;
create index if not exists memory_general_retrieval_fts_idx
  on public.memory_general using gin (retrieval_fts);

alter table public.memory_sport
  add column if not exists retrieval_fts tsvector
  generated always as (to_tsvector('simple'::regconfig,
    coalesce(content, '') || ' ' || coalesce(primary_subject, '') || ' ' ||
    coalesce(subjects::text, '') || ' ' || coalesce(relationships::text, '') || ' ' ||
...
create index if not exists memory_sport_retrieval_fts_idx
  on public.memory_sport using gin (retrieval_fts);

alter table public.memory_work
  add column if not exists retrieval_fts tsvector
  generated always as (to_tsvector('simple'::regconfig,
    coalesce(content, '') || ' ' || coalesce(primary_subject, '') || ' ' ||
    coalesce(subjects::text, '') || ' ' || coalesce(relationships::text, '') || ' ' ||
...
create index if not exists memory_work_retrieval_fts_idx
  on public.memory_work using gin (retrieval_fts);

alter table public.memory_research
  add column if not exists retrieval_fts tsvector
  generated always as (to_tsvector('simple'::regconfig,
    coalesce(question, '') || ' ' || coalesce(summary, '') || ' ' ||
    coalesce(meaning, '') || ' ' || coalesce(findings::text, '') || ' ' ||
    coalesce(relationships::text, '') || ' ' || coalesce(lessons::text, '')
  )) stored;
create index if not exists memory_research_retrieval_fts_idx
  on public.memory_research using gin (retrieval_fts);

alter table public.episodic_memories
  add column if not exists retrieval_fts tsvector
  generated always as (to_tsvector('simple'::regconfig,
    coalesce(event_date, '') || ' ' || coalesce(category, '') || ' ' || coalesce(summary, '')
  )) stored;
create index if not exists episodic_memories_retrieval_fts_idx
  on public.episodic_memories using gin (retrieval_fts);

alter table public.identity_anchors
  add column if not exists retrieval_fts tsvector
  generated always as (to_tsvector('simple'::regconfig,
    coalesce(key, '') || ' ' || coalesce(value, '')
  )) stored;
create index if not exists identity_anchors_retrieval_fts_idx
  on public.identity_anchors using gin (retrieval_fts);
