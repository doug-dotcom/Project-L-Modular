-- Avoid serialising unused legacy columns while ranking database candidates.
...
begin
  ...
  loop
    ...
    if v_source_column = '' then
      ...
      $sql$, v_row_expression, v_table, v_priority_expression, v_table);
    else
      v_union_sql := v_union_sql || format($sql$
      select
        public.project_l_candidate_row(
          %s,
          %L,
          case when source.role in ('user', 'assistant') then source.role else 'unknown' end,
          case when source.role in ('user', 'assistant') then 'raw_catchall' else 'unlinked' end,
          $1
        ) as row_data,
        ts_rank_cd(m.retrieval_fts, $1) as rank,
        %s as priority,
      ...
      where m.retrieval_fts @@ $1
    $sql$, v_row_expression, v_table, v_priority_expression, v_table, v_source_column);
    end if;
  end loop;

  v_sql := format($sql$
    select coalesce(jsonb_agg(row_data order by rank desc, priority desc, created_at desc), '[]'::jsonb)
    from (
      select row_data, rank, priority, created_at
