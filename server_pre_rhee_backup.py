...
# ============================================================
# PROJECT L BEHAVIOUR LAYER V1
# Added by Behaviour Layer AODS
# ============================================================

try:
    from pydantic import BaseModel as _BehaviourBaseModel
    from memory.behaviour.engine import (
        record_behaviour_from_exchange as _record_behaviour_from_exchange,
        build_behaviour_patterns as _build_behaviour_patterns,
        get_recent_behaviour_events as _get_recent_behaviour_events,
        truth_implementation_status as _truth_implementation_status,
    )

    class BehaviourIngestRequest(_BehaviourBaseModel):
        user_text: str
        assistant_text: str = ""

    @app.post("/behaviour/ingest")
    def behaviour_ingest(req: BehaviourIngestRequest):
        return _record_behaviour_from_exchange(
    ...
    def behaviour_recent(limit: int = 20):
        return {
            "events": _get_recent_behaviour_events(limit)
        }

    @app.get("/behaviour/patterns")
    def behaviour_patterns(limit: int = 200):
        return _build_behaviour_patterns(limit)

    @app.get("/behaviour/truth-status")
    def behaviour_truth_status():
        return _truth_implementation_status()

except Exception as e:
    print("Behaviour Layer v1 endpoint load error:", str(e))
